...
The root UI sends actions to `RecordingViewModel`. The ViewModel owns the countdown in `startJob`. Until the countdown reaches zero, it does not call the recording use case. The UI only renders `RecordingState.startCountdownSeconds`.
...
- `../app/src/main/java/com/tomasrepcik/sensorbox/domain/measurement/MeasurementControlUseCase.kt`
...
- `../app/src/main/java/com/tomasrepcik/sensorbox/domain/measurement/PhoneRecordingController.kt`
...
RecordingViewModel
  ...
  -> PairedRecordingCoordinator
       ...
       -> SendWearCommandUseCase (only when Wear sources are selected)
```

`PairedRecordingCoordinator` owns one small state gate. It starts the phone first, sends a direct Wear start command, and cleans up both sides if Wear fails. It has no prepare/commit/rollback protocol and no mutex. Model-to-command conversions live beside the models in `PairedRecordingModel.kt`.

Check the stale-command rule: a stop received from Wear only affects the matching active session.

Also check the disconnected-timer rule. Once the Wear start command has been sent, losing its result does not tear down the phone recording. The phone and Wear services receive the same duration and each local `RecordingEngine` schedules its own stop. Automatic peer notifications are best effort after local cleanup.
...
- `../app/src/main/java/com/tomasrepcik/sensorbox/domain/paired/PhoneWearMessageDispatcher.kt`
- `../app/src/main/java/com/tomasrepcik/sensorbox/domain/paired/PhoneWearCommandHandler.kt`
- `../app/src/main/java/com/tomasrepcik/sensorbox/domain/paired/WearRecordingResultInbox.kt`
...
`MeasurementService` routes lifecycle and intents. `MeasurementHostResources` owns Android host concerns such as notification, wake lock, receiver, and alarm. The factory assembles a controller. The controller translates the launch request into a direct recording plan. The engine then starts or stops ordered `RecordingSource` instances and knows nothing about Android.

The key engine seam is `RecordingSource`. It is intentionally small: `start(spec)` and `stop()`.
...
- `../sensorservices/src/main/java/com/tomasrepcik/sensorbox/sensorservices/serviceController/SessionRecordingSource.kt`
...
- `../sensorservices/src/main/java/com/tomasrepcik/sensorbox/sensorservices/handlers/measurements/ExtraInfoHandler.kt`
