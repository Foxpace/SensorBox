...
fun RecordingSetupScreen(
    ...
    Column(modifier.fillMaxSize()) {
        Box(Modifier.weight(1f)) {
            RecordingSetupContent(state, onIntent, onBack)
        }
...
@Composable
private fun RecordingSetupContent(state: RecordingState, onIntent: (RecordingIntent) -> Unit, onBack: () -> Unit) {
    SensorBoxBackScreen(
...
@Composable
private fun RecordingArchivePanel(path: String?, onChoose: () -> Unit) {
    Row(
...
@Composable
private fun RecordingSetupTopBar(
    state: RecordingState,
    onIntent: (RecordingIntent) -> Unit,
    modifier: Modifier = Modifier,
) {
        ...
        Box(
            ...
            SensorBoxPrimaryButton(
                label = state.startCountdownSeconds?.let { seconds ->
                    ...
                    if (state.isStarting) R.string.starting_recording else R.string.start_recording,
                ),
