...
import com.tomasrepcik.sensorbox.measurements.browser.SensorChartTimeWindow
import com.tomasrepcik.sensorbox.measurements.storage.SensorSeriesSample
import com.tomasrepcik.sensorbox.recording.preview.formatSensorChartDuration
import com.tomasrepcik.sensorbox.recording.preview.selectSensorSamplesInTimeWindow
...
@Composable
private fun StoredChartControls(onIntent: (MeasurementBrowserIntent) -> Unit) {
    Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
        TextButton(onClick = { onIntent(MeasurementBrowserIntent.ZoomSensorChartTimeWindow(0.5f, 0.5f)) }) {
            Text(stringResource(R.string.sensor_chart_zoom_out))
        }
        TextButton(onClick = { onIntent(MeasurementBrowserIntent.ShowEntireSensorChartTimeRange) }) {
            Text(stringResource(R.string.sensor_chart_show_all))
        }
        TextButton(onClick = { onIntent(MeasurementBrowserIntent.ZoomSensorChartTimeWindow(2f, 0.5f)) }) {
